...
export const schema = {
  name: "gdrive_search",
  description:
    "Search for files in Google Drive using plain keywords (e.g., `file name`). The tool wraps your text in a Drive query of the form `name contains '<keywords>' and trashed = false`.",
  inputSchema: {
    type: "object",
    properties: {
      query: {
        type: "string",
        description:
          "Plain keywords to match against file names. Do not include Drive operators such as `name contains`; the tool adds them automatically.",
      },
      pageToken: {
        type: "string",
